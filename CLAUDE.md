...
- `docs/HALLAZGOS_CLAVE.md` — decision-critical digest of the team's findings (official vs preliminary `S_max`/`S_min`, `Δu` convention, ΔS cross-validated, one-hot encoding, drought-regime `u=0`). **Auto-imported** (always loaded).
...
- `docs/ANALISIS_DP_Y_RESULTADOS.md` — DP complexity/timing + current results digest (baselines/DP, ΔSRS, feasibility) + §8 qubit-scaling & droppable constraints + §9 QAOA (Fase 3) + §10 QAOA-por-etapas/chunking. **On-demand.** Raw data in `results/runs_summary.csv`.
- `docs/AUDITORIA_DATOS_Y_BRUTE.md` — brute-force + data audit: brute correctness vs `check_constraints`, unit/`S_obs` cross-checks, official ΔS sign convention, why the T12/L3 optimum is `u=0` (drought regime). **On-demand.**
...
- **Active implementation guidance:** `docs/SPEC_IMPLEMENTACION_QUBO.md` (the real formulation/conventions, already implemented in `scripts/falcon_*.py`) + `docs/ANALISIS_DP_Y_RESULTADOS.md` (results). These, not any single input dump, define how the pipeline actually works.
